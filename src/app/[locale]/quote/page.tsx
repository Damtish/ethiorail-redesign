...
export default async function QuotePage({params}: Props) { const {locale} = await params; if (!isAppLocale(locale)) notFound(); const content = load(locale); setRequestLocale(locale); return <ContactQuotePage kind="quote" content={content} breadcrumbs={[{label: locale === "am" ? "መነሻ" : "Home", href: "/"}, {label: content.hero.eyebrow}]} />; }
